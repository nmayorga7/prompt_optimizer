...
from . import ai_client, prompts, ui
from .types import (
...
def update_context_understanding(
    state: ConversationState, parsed: Dict[str, str]
) -> None:
...
def generate_optimized_prompt(
    model: str, state: ConversationState
) -> OptimizationResult:
...
"""
    ...
    with ui.show_progress(
        "Analyzing conversation and crafting the perfect prompt...", style="purple"
    ):
        response = ai_client.chat_completion(
            model=model,
            messages=[
...
def generate_test_cases(
    model: str, optimized_prompt: str, context_understanding: ContextUnderstanding
) -> List[TestCase]:
...
def add_refinement_to_conversation(
    state: ConversationState, result: OptimizationResult, feedback: str
) -> None:
    ...
    while True:
        ...
        if choice == "1":
            ...
            test_cases = generate_test_cases(
                model, result["optimized_prompt"], state["context_understanding"]
            )
