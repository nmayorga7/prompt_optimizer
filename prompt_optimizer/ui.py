...
def show_refinement_header() -> None:
    ...
    console.print(
        "[cyan]💭 Let's refine your prompt together. I'll ask targeted questions to understand your needs.[/cyan]"
    )
...
def show_optimized_prompt(result: OptimizationResult) -> None:
    ...
    console.print(
        "\n",
        Rule(
            "[bold purple]🎯 Generating Optimized Prompt[/bold purple]", style="purple"
        ),
    )
...
def show_test_cases(test_cases: List[TestCase]) -> None:
    ...
    console.print(
        "[dim]💡 Try these test cases with your optimized prompt to ensure it works as expected![/dim]"
    )
